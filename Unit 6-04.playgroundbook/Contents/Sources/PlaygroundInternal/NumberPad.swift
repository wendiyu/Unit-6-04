...
class NumberPad: UIControl {
        ...
        for row in 0...3 {
            for column in 0...2 {
                ...
                if row == 0 && column > 0 {
                    ...
                    layer.addSublayer(GridLine(from: CGPoint(x: gridLineX, y: 0.0), to: CGPoint(x: gridLineX, y: 4.0 * buttonSize.height)))
                }
            }
            
            if row > 0 {
                let gridLineY = CGFloat(row) * buttonSize.height
                layer.addSublayer(GridLine(from: CGPoint(x: 0.0, y: gridLineY), to: CGPoint(x: 3.0 * buttonSize.width, y: gridLineY)))
            }
        }
    }
    ...
    private func updateSwipeActions() {
        ...
        if allowsFloats {
            let decimalAxText = NSLocalizedString("Insert decimal point", comment: "Accessibility text for the decimal button")
            swipeButton.setPrimaryActionImage(UIImage(named: "Decimal"), accessibilityDescription: decimalAxText)
            
            let signAxText = NSLocalizedString("Change number's sign", comment: "Accessibility text for the number sign change button")
            swipeButton.setSecondaryActionImage(UIImage(named: "Negative Positive"), accessibilityDescription: signAxText)
            ...
            swipeButton.setPrimaryActionImage(UIImage(named: "Negative Positive"), accessibilityDescription: "Change number's sign")
    ...
    @objc private func numberTapped(_ sender: NumberPadButton) {
        guard let number = numberButtons.index(of: sender) else {
            return
        }
